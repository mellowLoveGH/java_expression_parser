...
//		return null;
		// this method should return a new number leaf with value n created using your
		// constructor
		// if you've used the abstract class approach you will probably need something
		// like
		// return new NumLeaf(n);
		// if you've used an ExpTree class that stores the node kind you will probably
		// need something like
		// return new ExpTree(ExpTree.numNode, n, null, null);
	}
...
//		return null;
		// this method should return a new plus node with children l and r created using
		// your constructor
		// if you've used the abstract class approach you will probably need something
		// like
...
//		return null;
	...
	// the next three methods need to be modified for part 6 of the assignment - do
	// not modify them if you have not attempted part 6
	
	
	/*
	 * here the methods in 'ExpTree' class are generalized for all the following cases
	 */
	
	static ExpTree makeLetTree(ExpTree l, ExpTree r) { // remove the following line if you modify this method; leave it
														// here if you do not attempt part 6
//		System.out.println("Part 6 not attempted");
		return new ExpTree(ExpTree.let, l, r);
//		return null;
		// this method should return a new let node with children l and r
	}

	static ExpTree makeAndTree(ExpTree l, ExpTree r) {
		return new ExpTree(ExpTree.and, l, r);
//		return null;
		// this method should return a new and node with children l and r
	}

	// here 's' is the identifier
	// the result of expression 't' will be assigned to 's'
	// use the '=' node as the parent of 's' and 't'
	static ExpTree makeDefTree(String s, ExpTree t) {
		ExpTree l = makeIdLeaf(s);
		return new ExpTree(ExpTree.equal, l, t);
//		return null;
		// this method should return a new definition node with identifier s and child t
		// if your definition nodes have 2 children you should put a new id leaf
		// containing c in the left child and use t as the right child
	}
...
class Lexer {
	static final int err = 0, num = 1, id = 2, plus = 3, minus = 4, times = 5, div = 6, mod = 7, lp = 8, rp = 9,
			semico = 10, let = 11, and = 12, in = 13, eq = 14, pow = 15;
	int token;
	String idval;
	int numval;
	private String line = "";
		...
		if (line.length() == 0)
			...
			default:
				if (Character.isDigit(line.charAt(0))) {
					...
					line = line.substring(i).trim();
				} else if (line.length() >= 3 && line.charAt(0) == 'a' && line.charAt(1) == 'n' && line.charAt(2) == 'd'
						&& (line.length() == 3 || !Character.isLowerCase(line.charAt(3)))) {
					token = and;
					line = line.substring(3).trim();
				} else if (line.length() >= 3 && line.charAt(0) == 'l' && line.charAt(1) == 'e' && line.charAt(2) == 't'
						&& (line.length() == 3 || !Character.isLowerCase(line.charAt(3)))) {
					token = let;
					line = line.substring(3).trim();
				} else if (line.length() >= 2 && line.charAt(0) == 'i' && line.charAt(1) == 'n'
